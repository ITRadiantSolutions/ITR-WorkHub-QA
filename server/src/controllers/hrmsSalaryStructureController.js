...
const validateComponents = (components) => {
  ...
  for (const c of components) {
    ...
    if (!["earning", "deduction"].includes(c.type)) return "each component's type must be 'earning' or 'deduction'";
...
export const upsertSalaryStructure = async (req, res) => {
  ...
  const structure = await SalaryStructure.findOneAndUpdate(
    { employee: employeeId },
    { components: cleanComponents, effectiveFrom: effectiveFrom || new Date(), updatedBy: req.user._id },
