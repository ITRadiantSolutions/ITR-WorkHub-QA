...
describe("startOnboarding", () => {
  ...
  it("creates a checklist from the default items and notifies the employee", async () => {
...
describe("setOnboardingItem", () => {
  const makeOnboarding = (items) => {
    ...
    return { _id: oid(), items, status: "in_progress", save: vi.fn().mockResolvedValue(undefined) };
  };

  it("404s when onboarding isn't found", async () => {
    Onboarding.findById.mockResolvedValue(null);
  ...
  it("404s when the checklist item isn't found", async () => {
    ...
    Onboarding.findById.mockResolvedValue(onboarding);
  ...
  it("marks the checklist completed once every item is done", async () => {
    const item = { _id: oid(), label: "Offer accepted", done: false };
    const onboarding = makeOnboarding([item]);
    Onboarding.findById.mockResolvedValueOnce(onboarding).mockReturnValueOnce(makeQuery({}));
  ...
  it("keeps status in_progress when other items are still pending", async () => {
