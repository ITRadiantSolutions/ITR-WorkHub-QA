...
const STATUS_ICON = {
  approved: { icon: "CheckCircle", cls: "text-emerald-600" },
  submitted: { icon: "Clock", cls: "text-amber-500" },
  rejected: { icon: "X", cls: "text-red-500" },
  needs_edit: { icon: "Edit", cls: "text-amber-500" },
  draft: { icon: "Clock", cls: "text-slate-300" },
  not_submitted: { icon: "Clock", cls: "text-slate-300" },
};

export function TimesheetStatusSection() {
  const [selected, setSelected] = useState(new Set(["approved"]));
  const [data, setData] = useState({ weekStart: new Date(), rows: [] });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    API.get("/hr/timesheet-status", { params: { status: [...selected].join(",") } })
      .then((res) => setData(res.data))
      .catch(() => toast.error("Failed to load timesheet status"))
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <h3 className="font-bold text-slate-900">Timesheet Status</h3>
        <div className="flex items-center gap-2 flex-wrap">
          {STATUS_FILTERS.map((f) => (
            <label key={f.key} className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 cursor-pointer">
              <input type="checkbox" checked={selected.has(f.key)} onChange={() => toggleFilter(f.key)} className="accent-indigo-600" />
              {f.label}
            </label>
          ))}
        </div>
      ...
      {loading ? (
        ...
        <div className="p-8 text-center text-slate-400 text-sm">No employees match this filter.</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left px-3 py-2 font-bold text-slate-500 text-xs uppercase tracking-wide">Employee</th>
                {DAY_LABELS.map((d, i) => (
                  <th
                    key={d}
                    className={`px-2 py-2 font-bold text-center text-xs uppercase tracking-wide ${isSameDay(new Date(weekStart.getTime() + i * 86400000), today) ? "text-indigo-600" : "text-slate-500"}`}
                  >
                    {d}
                    <div className="text-[10px] font-normal text-slate-400 normal-case">{fmtShort(new Date(weekStart.getTime() + i * 86400000))}</div>
                  </th>
                ))}
                <th className="px-3 py-2 font-bold text-slate-500 text-xs uppercase tracking-wide text-center">Total</th>
                <th className="px-3 py-2" />
              </tr>
            </thead>
            <tbody>
              {data.rows.map((r) => {
                const style = STATUS_ICON[r.status] || STATUS_ICON.not_submitted;
                const StatusIcon = Icons[style.icon];
                return (
                  <tr key={r.userId} className="border-b border-slate-50 last:border-0">
                    ...
                    {r.dayTotals.map((v, i) => (
                      <td key={i} className="px-2 py-2.5 text-center">
                        {v > 0 ? (
                          <span className="inline-block min-w-[2.25rem] px-1.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 font-semibold text-xs tabular-nums">
                            {v.toFixed(1)}
                          </span>
                        ) : (
                    ...
                    <td className="px-3 py-2.5 text-center font-bold text-indigo-700 tabular-nums">{r.total.toFixed(1)}</td>
                    <td className="px-3 py-2.5 text-center">
                      {StatusIcon && (
                        <span className={`inline-flex ${style.cls}`}>
                          <StatusIcon />
                        </span>
                      )}
                    </td>
      ...
      <div className="flex justify-center pt-3 mt-2 border-t border-slate-50">
        <button
          onClick={() => toast.info("Full team review page is coming soon")}
          className="px-4 py-2 rounded-xl border border-indigo-200 text-indigo-600 text-sm font-semibold hover:bg-indigo-50 transition"
        >
          View All Timesheets
        </button>
      </div>
