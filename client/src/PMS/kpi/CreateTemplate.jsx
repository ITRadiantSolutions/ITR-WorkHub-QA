  ...
  const handleFinalSubmit = async () => {
    try {
      if (selectedKras.length === 0) {
        setError("Select at least one KRA");
        return false;
      }
      ...
      const payload = {
        name: templateName,
        functionalKras: selectedKras.filter((k) => k.type === "functional"),
        organizationalKras: selectedKras.filter((k) => k.type === "organizational"),
  ...
  const handleAssignedSubmit = async () => {
    try {
      ...
      if (savedKras.length === 0)
        return setError("Save all KRAs before submitting");
      ...
      const payload = {
        templateId: editingTemplateId || templateId,
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex items-center gap-3 bg-purple-50 px-4 py-2 rounded-xl border border-purple-100">
              ...
              <span
                className={`text-lg font-bold ${remainingWeight === 0 ? "text-emerald-500" : remainingWeight < 0 ? "text-red-500" : "text-purple-600"}`}
              >
                {remainingWeight}%
              </span>
            </div>
          ...
          {/* <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 overflow-x-auto">
            <div className="flex justify-between items-center w-full min-w-[500px] relative px-4">
              ...
              {[
                { num: 1, label: "Create Template" },
                { num: 2, label: "Assign Targets" },
                { num: 3, label: "Configure Weights" },
              ].map((s) => {
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (templateNameError) setTemplateNameError(false); // remove red
                      }}
                      placeholder="e.g. Q3 Engineering Core"
                      className={`w-full px-5 py-4 text-sm font-medium text-gray-800 rounded-2xl transition-all
  ${templateNameError
                          ? "border-red-400 bg-red-50 focus:ring-red-400"
                          : "bg-gray-50 border-gray-200 focus:bg-white focus:ring-purple-500"
                        }
`}
                    />
                  </div>
  ...
  ${kraTypeError ? "border-red-400 bg-red-50" : "border-gray-200"}`}
                ...
                <div className="flex justify-end mb-4">
                  <motion.button
                    onClick={() => navigate("/kra-builder?from=create_template")}
            ...
            <div className="mt-8 pt-6 border-t border-gray-100 flex items-center justify-between">
                ...
                <button
                  onClick={async () => {
                    if (!templateName.trim()) {
                      ...
                      setTemplateNameError(true);   // 🔥 highlight field
                      return;
                    }
                    ...
                    try {
                      ...
                      resetTemplateBuilder();
                      navigate("/available_template");  // 🔥 CHANGE ROUTE

                    } catch (err) {
                      console.error(err);
                    }
                  }}
                  className="px-8 py-2.5 rounded-xl text-sm font-bold text-white bg-purple-600 hover:bg-purple-700 transition-all shadow-md"
                >
                  Save Template
                </button>
              )}
              ...
              {step === 3 && selectedKras.length > 0 && (
                <div className="flex gap-3">
                  ...
                  {/* ✅ NEW BUTTON → SAVE ONLY */}
                  {/* <button
                    onClick={async () => {
                      // same validations
                      const unsavedKra = selectedKras.some((k) => !k.isSaved);
                      if (unsavedKra) {
                        setError("Please save all KRAs before saving template");
                        return;
                      }

                      if (totalKraWeight !== 100) {
                        setError("Total KRA weight must be 100%");
                        return;
                      }

                      try {
                        await handleFinalSubmit();   // ✅ ONLY SAVE
                        resetTemplateBuilder();
                        setStep(1);
                      } catch (err) {
                        console.error(err);
                    ...
                    className="px-6 py-2.5 rounded-xl text-sm font-bold text-white bg-violet-600 hover:bg-violet-700 transition-all shadow-md"
                  >
                    Save Template Only
                  </button> */}

                  {/* 🔥 EXISTING BUTTON (UNCHANGED LOGIC)*/}
                  <button
                    onClick={() => {
                      const unsavedKra = selectedKras.some((k) => !k.isSaved);
                      if (unsavedKra) {
                      ...
                      if (totalKraWeight !== 100) {
                        setError("Total KRA weight must be exactly 100%");
                        return;
                      }
                      ...
                      for (let kra of selectedKras) {
                        if (kra.kpis.length > 0) {
                          ...
                          if (zeroKpi) {
                            setError(`Each KPI must have weight > 0 for "${kra.name}"`);
                            return;
                          }
                        }
                      }
                    ...
                    className="px-8 py-2.5 rounded-xl text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition-all shadow-md flex items-center gap-2"
                  >
